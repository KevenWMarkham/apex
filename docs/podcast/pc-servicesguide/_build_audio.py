"""
Build podcast audio from markdown scripts (pc-servicesguide variant).

Parses **KEVEN:** / **MORGAN:** dialogue lines, generates per-segment audio via
...
Voices:
  ...
  MORGAN = en-US-EmmaNeural    (female, cheerful, conversational — same family as Andrew)

Output: one MP3 per episode at podcast quality (24kHz mono, 48kbps).

Usage:
    python _build_audio.py 01-why-services-are-data-first.md
...
VOICE_MORGAN = "en-US-EmmaNeural"    # Cheerful, Clear, Conversational — Conversation/Copilot tuned

# Rate / pitch tuning. Both voices are Conversation/Copilot-family — same
# "natural speaker" register. Mild speed-up on Morgan for engaged cadence.
RATE_KEVEN = "+0%"
RATE_MORGAN = "+2%"
PITCH_KEVEN = "+0Hz"
PITCH_MORGAN = "+0Hz"
...
# Match speaker line: **KEVEN:** or **MORGAN:** at start, content until next
# speaker, heading, or section break.
DIALOGUE_RE = re.compile(
    r"^\*\*(KEVEN|MORGAN):\*\*\s*([\s\S]*?)"
    r"(?=^\*\*(?:KEVEN|MORGAN):\*\*|^##|^---|\Z)",
    re.MULTILINE,
)
...
async def synth_episode(md_path: Path, out_dir: Path) -> Path:
    ...
    for i, (speaker, text) in enumerate(segments):
        if speaker == "KEVEN":
            ...
            voice, rate, pitch = VOICE_MORGAN, RATE_MORGAN, PITCH_MORGAN
...
EPISODES = [
    ...
    # v2 — Synthesis
    "12-what-the-catalog-becomes.md",
]
