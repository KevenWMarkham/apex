"""
Build podcast audio from markdown scripts.

Parses **KEVEN:** / **JORDAN:** dialogue lines, generates per-segment audio via
...
Usage:
    python _build_audio.py 01-the-moment.md
...
VOICE_KEVEN = "en-US-AndrewNeural"   # Warm, Confident, Authentic, Honest
VOICE_JORDAN = "en-US-BrianNeural"   # Approachable, Casual, Sincere

# Rate / pitch tuning — slight differentiation makes the two voices distinct
# even though they're both male US neural voices.
RATE_KEVEN = "+0%"
RATE_JORDAN = "+3%"   # Jordan slightly faster — fits the "skeptic pushing back" persona
PITCH_KEVEN = "+0Hz"
PITCH_JORDAN = "-2Hz" # Jordan slightly lower pitched for contrast
...
# Match speaker line: **KEVEN:** or **JORDAN:** at start, content until next
# speaker, heading, or section break.
DIALOGUE_RE = re.compile(
    r"^\*\*(KEVEN|JORDAN):\*\*\s*([\s\S]*?)"
    r"(?=^\*\*(?:KEVEN|JORDAN):\*\*|^##|^---|\Z)",
    re.MULTILINE,
)
...
async def synth_episode(md_path: Path, out_dir: Path) -> Path:
    ...
    for i, (speaker, text) in enumerate(segments):
        if speaker == "KEVEN":
            ...
            voice, rate, pitch = VOICE_JORDAN, RATE_JORDAN, PITCH_JORDAN
...
EPISODES = [
    # Original Sellers Guide arc
    "01-the-moment.md",
    "02-the-commercial-arc.md",
    "03-the-four-pillars.md",
    "04-the-seven-industries.md",
    "05-anchor-accounts-part-1.md",
    "06-anchor-accounts-part-2.md",
    "07-pursuit-motion-and-lessons.md",
    # Sellers Handbook weave (added 2026-05-13)
    "08-the-sellers-dream-and-the-six-platforms.md",
    "09-functional-area-discovery.md",
    "10-the-four-level-ladder-and-the-lab.md",
]
