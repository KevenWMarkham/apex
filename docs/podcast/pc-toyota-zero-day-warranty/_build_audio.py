"""
Build podcast audio from markdown scripts (pc-toyota-zero-day-warranty variant).

Parses **KEVEN:** / **MIA:** dialogue lines, generates per-segment audio via
edge-tts (Microsoft neural voices), then concatenates with ffmpeg.

Voices:
  KEVEN = en-US-AndrewNeural   (male, warm, Trilogy continuity host)
  MIA   = en-US-MichelleNeural   (female, natural register matching Andrew —
                                18 yr automotive account partner; Mfg-IT and
...
This is the seventh distinct voice pairing in the APEX podcast family:
  ...
  - Toyota:   Andrew + Michelle (Toyota Zero Day Warranty)  <-- this podcast

Output: one MP3 per episode at podcast quality (24kHz mono, 48kbps).

Usage:
    python _build_audio.py 01-the-zero-day-warranty-idea.md
...
VOICE_MIA   = "en-US-MichelleNeural"   # Natural neural voice matching Andrew's register

# Rate / pitch tuning. Mia reads at slightly slower-than-Keven pace —
# senior-account-partner deliberate cadence, comfortable with shop-floor reality.
RATE_KEVEN = "+0%"
RATE_MIA   = "-2%"
PITCH_KEVEN = "+0Hz"
PITCH_MIA   = "+0Hz"
...
# Match speaker line: **KEVEN:** or **MIA:** at start, content until next
...
EPISODES = [
    ...
    "03-the-24-step-agent-and-the-microsoft-platform.md",
    "04-nvidia-at-the-station.md",
    "05-omniverse-and-the-90-day-path.md",
]
