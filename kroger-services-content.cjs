...
|---|---|---|---|
| **Markdown Opportunity Agent** | POS, supplier cost, current shelf price, category margin targets | Ranked list of SKU-store pairs by margin exposure (current vs target) | Where is gross margin at risk this cycle? |
| **Price Elasticity Agent** | 84.51° elasticity coefficients, historical price-test data, basket-affinity signals | Per-SKU price-move impact curves with confidence bands | What happens if we move this price 1%, 3%, 5%? |
| **Competitive Index Monitoring Agent** | Competitor pricing feeds, banner positioning rules, KVI (Known Value Item) flags | Recommended response to a competitor move (match, ignore, undercut, repress) | How do we respond to a competitor's move on a watched SKU? |
| **Whitespace Identification Agent** | Vendor-funded promotion proposals, supplier-spend history, elasticity outputs | Counter-offer structures and participation recommendations | Do we accept, modify, or decline this VFP proposal? |
| **Store-Level Personalization Agent** | Basket data, BOPIS substitution history, out-of-stock signals | Substitution acceptance rates per SKU pair, assortment-rationalization candidates | Which SKUs can we discontinue without losing the basket? |
| **Product Clustering Agent** | All upstream agent outputs, merchant calendar, HITL acceptance history | Ranked merchant queue: top-N approve-or-reject items with rationale | Which 200 recommendations does the merchant see this week? |
...
A normal weekly cycle looks like this. The **Markdown Opportunity Agent** runs first against the freshest POS and cost data, producing a ranked list of where margin is most at risk against category targets. The **Price Elasticity Agent** consumes the at-risk list and overlays per-SKU price-move impact curves, narrowing the actionable subset. The **Competitive Index Monitoring Agent** runs in parallel against competitor-pricing feeds, producing a list of competitor-driven response candidates. The **Whitespace Identification Agent** runs against any open VFP proposals on the merchant's desk, scoring each against the elasticity outputs. The **Store-Level Personalization Agent** runs against the assortment side, flagging SKUs whose basket-substitutability allows discontinuation candidates. Finally, the **Product Clustering Agent** consumes all five upstream outputs, deduplicates and prioritizes them against the merchant's calendar (e.g., this week's category review meeting), and produces the ranked queue of 100-300 recommendations the merchant sees in Copilot. The composer is the only agent whose output the merchant directly sees; the upstream five are visible to the merchant only as the rationale behind a specific recommendation.
...
| Agent | Inputs | Outputs | Owns decision boundary |
|---|---|---|---|
| **Compliance Monitoring Agent** | Receiving event stream, supplier ASN lot-data, FSMA 204 KDE schema | Per-event compliance score, gap reports for missing KDEs | Are receiving events meeting FSMA 204 KDE requirements? |
| **Disposition Optimizer Agent** | FDA recall notice, Eventhouse TLC lineage, store and DC inventory positions, loyalty transaction attributions | Recall scope (units, lots, stores, customer cohorts), pull-plan sequence, customer-notification roster | When a recall posts, what is the exact scope and what is the pull plan? |
| **Supplier Optimization Agent** | Supplier ASN feeds, lot-data submission timeliness, KDE completeness, historical audit findings | Per-supplier data-quality score, exception lists, supplier-improvement recommendations | Which suppliers' lot-data submissions are timely, complete, and audit-defensible? |
| **Distribution and Fulfillment Agent** | DC receipt events, store-transfer events, store receiving events, POS scans, BOPIS pick events | Real-time TLC location map across DC-to-store-to-customer flow | Where is a given TLC right now across the DC-to-store-to-customer flow? |
| **Regulatory Compliance Agent** | FSMA 204 rule set, state regulatory variants (CA AB-660, NY traceability, etc.), Eventhouse TLC records | Per-lot regulatory-coverage stamp, jurisdictional gap flags, FDA-format export packages | Does this lot's data satisfy FSMA 204 plus applicable state variants? |
| **Inventory Holding Agent** | Active recall scope, hold reasons, retention SLAs, destruction-or-release decisions | Per-lot hold status, hold duration, release-or-destruction recommendation | Which lots are on hold, why, for how long, and what is the release-or-destruction decision? |
...
**Third attach (sometimes Wave 2, sometimes early Wave 3): RC-TTP-12 Promotion Effectiveness or RC-TTP-13 VFP Management.** Both attach to the Whitespace Identification Agent. Promotion Effectiveness is the measurement-and-learning loop; VFP Management is the supplier-facing workflow on top. Sequence depends on prospect priorities — a supplier-revenue-pressured CFO will lead with VFP; a merchant-team-pressured Chief Merchant will lead with Promotion Effectiveness.
...
- **The agent library.** The six-agent merchant-decision pod (Markdown Opportunity, Price Elasticity, Competitive Index Monitoring, Whitespace Identification, Store-Level Personalization, Product Clustering) is decision-class-shaped, not Kroger-shaped. The agents work for any grocer with the same merchant-decision boundaries; the inputs change, the agents do not.
...
- **Markdown Opportunity Agent** — where is gross margin at risk this cycle?
- **Price Elasticity Agent** — what happens if we move this price 1%, 3%, 5%?
- **Competitive Index Monitoring Agent** — how do we respond to a competitor's move on a watched SKU?
- **Whitespace Identification Agent** — do we accept, modify, or decline this VFP proposal?
- **Store-Level Personalization Agent** — which SKUs can we discontinue without losing the basket?
- **Product Clustering Agent** — which 200 recommendations does the merchant see this week?
...
|---|---|---|---|
| **Markdown Opportunity Agent** | Top-tier reasoning (e.g., Claude Sonnet, GPT-4o) | Multi-factor reasoning across margin targets, supplier cost, KVI flags, category strategy; first-pass agent that anchors the entire week | 4-12K input, 1-3K output per cycle |
| **Price Elasticity Agent** | Top-tier reasoning (e.g., Claude Sonnet, GPT-4o) | Probabilistic reasoning over elasticity coefficients, basket-affinity, confidence bands; output drives downstream pricing decisions | 6-15K input, 2-4K output per SKU |
| **Competitive Index Monitoring Agent** | Top-tier reasoning (e.g., Claude Sonnet, GPT-4o) | Game-theoretic reasoning over competitor postures, banner positioning, KVI implications; latency-sensitive (must respond inside the merchant's review window) | 4-10K input, 1-2K output per move |
| **Whitespace Identification Agent** | Mid-tier reasoning (e.g., GPT-4o-mini, Claude Haiku) | Bounded decision class; structured input (VFP proposal), structured output (accept/modify/decline + counter-offer); benefits from fine-tuning over time | 2-6K input, 1-2K output per proposal |
| **Store-Level Personalization Agent** | Mid-tier reasoning (e.g., GPT-4o-mini, Claude Haiku) | Pattern-recognition over basket data; not a reasoning-intensive decision but a high-volume one; ideal candidate for fine-tuning | 3-8K input, 1K output per candidate SKU |
| **Product Clustering Agent** | Smaller model with strong tool-calling (e.g., Claude Haiku, fine-tuned smaller) | Orchestration and ranking, not deep reasoning; consumes upstream agent outputs and merchant calendar; tool-calling dominates over reasoning | 8-20K input, 2-5K output per merchant queue |

The routing logic — *Markdown Opportunity, Price Elasticity, Competitive Index Monitoring at the top tier; Whitespace Identification, Store-Level Personalization at mid tier; Product Clustering at small with strong tool-calling* — is the starting posture. It changes as agents mature down the path described in Section 11.4.

## 11.3 Tool calling and MCP integration (preview Ch 12)

Agents do not query data sources directly; they call tools exposed by MCP servers. This decoupling is what lets us swap models without rebuilding the agent code, and it is what lets the same agent code serve multiple banners with different backend integrations. The Markdown Opportunity Agent does not know whether the price-point data lives in Fabric Direct Lake, in an Eventhouse query, or in a mirrored Azure SQL instance — it calls a tool named *get-current-price-points-for-category* with typed input, and the MCP server resolves the call against whatever backend currently serves the data.

When the model class behind the Markdown Opportunity Agent changes (an upgrade from one top-tier model to another, a fine-tuning step, a distillation step), the tool-calling contract does not change. The agent code adapts to the new model's tool-calling syntax (which is itself converging across model providers), but the tools themselves stay stable. This is the structural property that makes the model-routing table above changeable rather than brittle. Chapter 12 covers the MCP layer in depth.
...
- **Mature mixed fleet (Wave 2 to mid Wave 3):** $0.005–$0.02 per decision. Mix of top-tier on the reasoning-heavy agents, fine-tuned mid-tier on the Whitespace Identification and Store-Level Personalization agents. Volume has scaled to thousands of decisions per cycle across multiple categories.
- **Distilled production fleet (mid Wave 3 onward):** sub-cent per decision. Top-tier reserved for the highest-stakes reasoning calls (Competitive Index Monitoring under competitive pressure, Markdown Opportunity for at-risk categories); distilled small models for the high-volume orchestration and pattern-recognition work. Volume at full Kroger scale is tens of thousands of decisions per day.
...
Agents call tools **on-behalf-of** a human user (or a service principal for unattended scenarios). **Entra** issues the scoped tokens; the audit row captures the user identity. The Markdown Opportunity Agent invoked from Cassidy's Copilot session calls tools with Cassidy's identity in the token; the audit row attributes the recommendation chain back to Cassidy; if Cassidy lacks the auth scope for a particular tool, the tool call fails with an authorization error.

This is the structural pattern that makes the agent's actions attributable to a human. Without it, agent actions are ambiguous from an audit perspective — they were taken by the system, not by anyone in particular. With on-behalf-of identity, every action attributes; the audit row is defensible; the security and compliance posture survives review. For unattended scenarios (e.g., an overnight Markdown Opportunity batch run with no merchant in the loop), the agent runs as a service principal with explicit auth scopes, and the audit row attributes the recommendation chain back to the service principal — which is itself owned by a named team and bounded by explicit scope.
...
**Slide 5 — The six agents.** The agent table from Chapter 5: Markdown Opportunity, Price Elasticity, Competitive Index Monitoring, Whitespace Identification, Store-Level Personalization, Product Clustering. Two minutes. The decision-class-first naming is what makes this slide land; the executive sees that the agents are organized around their merchant team's actual decision boundaries, not around vendor-defined product categories.
...
| **Agent library** | High | The six agents (Markdown Opportunity, Price Elasticity, Competitive Index Monitoring, Whitespace Identification, Store-Level Personalization, Product Clustering) are decision-class-shaped, not Kroger-shaped. Every grocer's merchant team has the same six decision boundaries; the agents' reasoning patterns, model-routing logic, prompt structure, and HITL contract all carry forward. The eval golden sets adapt at the margin (different category mix, different competitor set) but the core eval architecture transfers. Likewise, RC-E2E-09's six-agent traceability fleet (Compliance Monitoring, Disposition Optimizer, Supplier Optimization, Distribution and Fulfillment, Regulatory Compliance, Inventory Holding) is FSMA-204-shaped, not Kroger-shaped — every grocer's regulatory environment requires the same six decision boundaries. |
