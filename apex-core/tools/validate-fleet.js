...
/**
 * Validate a fleet registry against per-edition manifests.
 *
 * @param {object} registry       Parsed fleet-registry.json (L3).
...
export function validateFleet(registry, editionManifests, opts = {}) {
  ...
  if (!registry || typeof registry !== 'object' || Array.isArray(registry)) {
    return {
      findings: [{ severity: 'critical', rule: 'FLEET-SHAPE-ROOT', path: '$', message: 'fleet registry must be a JSON object' }],
      drift
    };
  }
  ...
  if (!Array.isArray(registry.accounts)) {
    findings.push({ severity: 'critical', rule: 'FLEET-SHAPE-ACCOUNTS', path: '$.accounts', message: 'accounts must be an array' });
  }
  ...
  for (const [i, acc] of accounts.entries()) {
    ...
    if (!acc.account_id || !ACCOUNT_ID_RE.test(acc.account_id)) {
      findings.push({
        severity: 'critical', rule: 'FLEET-ACCOUNT-ID',
    ...
    if (seenIds.has(acc.account_id)) {
      findings.push({
        severity: 'critical', rule: 'FLEET-DUPLICATE-ID',
        path: `${aPath}.account_id`,
    ...
    for (const bump of REQUIRED_POLICY_BUMPS) {
      if (!policy[bump]) {
        findings.push({
          severity: 'critical', rule: 'FLEET-POLICY-MISSING',
        ...
        findings.push({
          severity: 'critical', rule: 'FLEET-POLICY-GATE',
          path: `${aPath}.auto_upgrade_policy.${bump}`,
    ...
    if (!manifest) {
      findings.push({
        severity: 'critical', rule: 'FLEET-EDITION-UNKNOWN',
    ...
    for (const [j, p] of (acc.pinned_schemas ?? []).entries()) {
      ...
      if (!semver.valid(p.version)) {
        findings.push({
          severity: 'critical', rule: 'FLEET-PIN-SEMVER',
          path: `${pPath}.version`,
      ...
      if (!catalog) {
        findings.push({
          severity: 'critical', rule: 'FLEET-PIN-UNKNOWN-SCHEMA',
          path: `${pPath}.code`,
      ...
      if (semver.gt(catalog, p.version)) {
        ...
        findings.push({
          severity: 'critical', rule: 'FLEET-PIN-AHEAD-OF-CATALOG',
          path: `${pPath}.version`,
