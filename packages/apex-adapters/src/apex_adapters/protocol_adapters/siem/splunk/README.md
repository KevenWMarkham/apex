# siem.splunk — Splunk adapter

**Satisfies:** `AuditLedger`, `Observability`
**Status:** Stub. Concrete implementation builds per-engagement.

## Purpose

Client-mandated parallel-write to Splunk SIEM.
...
1. Implement `client.py` wrapping the Splunk SDK
2. Provide IaC for adapter-side resources under `iac/`
3. Smoke test against a Splunk sandbox
