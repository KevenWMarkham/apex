"""
Build podcast audio from markdown scripts (pc-dtna-account variant).

Parses **KEVEN:** / **MARCUS:** dialogue lines, generates per-segment audio via
edge-tts (Microsoft neural voices), then concatenates with ffmpeg.

Voices:
  KEVEN  = en-US-AndrewNeural            (male, warm, Trilogy continuity host)
  MARCUS = en-US-BrianMultilingualNeural (male, approachable — senior account partner)

Output: one MP3 per episode at podcast quality (24kHz mono, 48kbps).

Usage:
    python _build_audio.py 01-the-dtna-account.md
...
VOICE_MARCUS = "en-US-BrianMultilingualNeural" # Senior account partner — industrial-account register

# Rate / pitch tuning. Marcus reads at slightly slower-than-Keven pace —
# senior-account-partner deliberate cadence.
RATE_KEVEN = "+0%"
RATE_MARCUS = "-2%"
PITCH_KEVEN = "+0Hz"
PITCH_MARCUS = "+0Hz"
...
# Match speaker line: **KEVEN:** or **MARCUS:** at start, content until next
# speaker, heading, or section break.
DIALOGUE_RE = re.compile(
    r"^\*\*(KEVEN|MARCUS):\*\*\s*([\s\S]*?)"
    r"(?=^\*\*(?:KEVEN|MARCUS):\*\*|^##|^---|\Z)",
    re.MULTILINE,
)
...
async def synth_episode(md_path: Path, out_dir: Path) -> Path:
    ...
    for i, (speaker, text) in enumerate(segments):
        if speaker == "KEVEN":
            ...
            voice, rate, pitch = VOICE_MARCUS, RATE_MARCUS, PITCH_MARCUS
...
EPISODES = [
    "01-the-dtna-account.md",
    "02-manufacturing-and-quality.md",
    "03-connected-vehicle-and-aftermarket.md",
    "04-the-ladder-for-dtna.md",
