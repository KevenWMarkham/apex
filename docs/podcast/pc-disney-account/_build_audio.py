"""
Build podcast audio from markdown scripts (pc-disney-account variant).

Parses **KEVEN:** / **RILEY:** dialogue lines, generates per-segment audio via
edge-tts (Microsoft neural voices), then concatenates with ffmpeg.

Voices:
  KEVEN = en-US-AndrewNeural           (male, warm, Trilogy continuity host)
  RILEY = en-US-EmmaMultilingualNeural (female, conversational, multilingual)

Output: one MP3 per episode at podcast quality (24kHz mono, 48kbps).

Usage:
    python _build_audio.py 01-the-disney-account.md
...
VOICE_RILEY = "en-US-EmmaMultilingualNeural"  # Senior account partner (multilingual-capable
                                              # for thematic fit with Disney's international footprint)

# Rate / pitch tuning. Riley reads slightly slower than Keven — senior account-
# partner cadence, deliberate not rushed.
RATE_KEVEN = "+0%"
RATE_RILEY = "-2%"
PITCH_KEVEN = "+0Hz"
PITCH_RILEY = "+0Hz"
...
# Match speaker line: **KEVEN:** or **RILEY:** at start, content until next
...
async def synth_episode(md_path: Path, out_dir: Path) -> Path:
    ...
    for i, (speaker, text) in enumerate(segments):
        if speaker == "KEVEN":
            ...
            voice, rate, pitch = VOICE_RILEY, RATE_RILEY, PITCH_RILEY
...
EPISODES = [
    "01-the-disney-account.md",
    "02-the-customer-experience-six.md",
    "03-the-operations-eight.md",
    "04-the-ladder-for-disney.md",
    "05-the-account-team-playbook.md",
    "06-the-curated-plays.md",
]
